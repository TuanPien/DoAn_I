...
class discount{
    ...
    public function show_discount($user_id){
        $query = "SELECT * FROM discount WHERE user_id = '$user_id' ORDER BY product_id ASC, discount_point ASC";
    ...
    public function admin_show_discount(){
        $query = "SELECT discount.*, user.user_email FROM discount INNER JOIN user ON discount.user_id = user.user_id 
