...
class campaign{
    ...
    public function time_check($campaign_id){
        ...
        $time_start = $temp['time_start'];
        $time_end = $temp['time_end'];
         // Chuyển đổi time_start và time_end thành timestamp nếu chúng chưa phải là timestamp
        $time_start = strtotime($time_start);
        $time_end = strtotime($time_end);

        // Kiểm tra nếu thời gian hiện tại bằng time_start
    ...
    public function cal_discount($campaign_id){
        ...
        // Khởi tạo mốc sản phẩm ban đầu bằng 0
        $discount_point = 0;

        // Tìm mức chiết khấu cho số lượng sản phẩm của chiến dịch
        ...
        // Tính toán lại thông tin chiến dịch
        ...
        // Cập nhật thông tin chiến dịch nếu có gì thay đổi
    ...
    public function show_campaign_by_product($product_id){
        ...
        if($result != false){
            ...
            return "Chưa có chiến dịch nào diễn ra";
        }
    }
    ...
    public function create_campaign(){
        ...
        if($check){
            ...
            echo "<h3 style=\"color: red;\">Bạn chưa thiết lập chiết khấu cho sản phẩm này</h3>";
        }
    }

    public function update_campaign($campaign_id, $user_id){
        // Cập nhật thời gian diễn ra chiến dịch
        $time_start = $_POST['time_start'];
        $time_end = $_POST['time_end'];

        $query = "SELECT * FROM campaign WHERE campaign_id = $campaign_id";
        $temp = $this -> db -> select($query) -> fetch_assoc();
        $start = $temp['time_start'];
        $end = $temp['time_end'];

        // Kiểm tra thời gian chiến dịch
        $time_check = $this -> time_check($campaign_id);
        if(is_bool($time_check) && $time_check == true && $time_start != $start){
            echo "<h3 style=\"color: red;\">Chiến dịch đang diễn ra, vui lòng không thay đổi thời gian bắt đầu</h3>";
        }else if(is_bool($time_check) && $time_check == false){
            echo "<h3 style=\"color: red;\">Chiến dịch đã kết thúc, vui lòng tạo chiến dịch mới</h3>";
        }else if(is_bool($time_check) && $time_check == true && $time_start == $start){
            // Lấy thời gian hiện tại
            $current_time = time();

            // Thêm 3 ngày vào thời gian hiện tại
            $new_time = strtotime('+3 days', $current_time);
            $str_end = strtotime($end);
            $str_time_end = strtotime($time_end);
            // Thời gian kết thúc tăng lên
            if($str_time_end >= $str_end){
                ...
                return $result;
            }else if($str_time_end < $str_end && $str_time_end < $new_time){
                // Thời gian kết thúc giảm đi quá sát, trong 3 ngày tính từ ngày hôm nay
                echo "<h3 style=\"color: red;\">Thời gian kết thúc quá sát hiện tại, vui lòng chọn ngày xa hơn</h3>";
            }else{
                // Thời gian kết thúc giảm đi nhưng lớn hơn thời hạn 3 ngày tính từ hôm nay
                $query = "UPDATE campaign SET time_end = '$time_end' WHERE campaign_id = $campaign_id";
                $result = $this -> db -> update($query);
                header("location:list_campaign.php?user_id=".$user_id);
                return $result;
    ...
    public function delete_campaign($campaign_id, $user_id){

        // Kiểm tra thời gian chiến dịch
        $time_check = $this -> time_check($campaign_id);

        //Nếu chiến dịch đã kết thúc, xoá chiến dịch và các thông tin vận chuyển
        if(is_bool($time_check) && $time_check==false){
            ...
            if($temp != false){
                while($row = $temp -> fetch_assoc()){
                    ...
                    $bin = $this -> db -> delete($query);

                    // Xoá chiến dịch
                    $query = "DELETE FROM campaign WHERE campaign_id = $campaign_id";
                    $result = $this -> db -> delete($query);
                    header('location:list_campaign.php?user_id='.$user_id);
                    return $result;
                }
            }
        }else if(is_bool($time_check) && $time_check==true){
            echo "<h3 style=\"color: red;\">Chiến dịch đang diễn ra</h3>";
        }else{ // Chiến dịch chưa diễn ra
            // Xoá chiến dịch
            $query = "DELETE FROM campaign WHERE campaign_id = $campaign_id";
            $result = $this -> db -> delete($query);
            header('location:list_campaign.php?user_id='.$user_id);
    ...
    public function delete_campaign_product($campaign_id){
